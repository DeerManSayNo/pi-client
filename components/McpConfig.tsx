  ...
  return <div role="dialog" aria-modal="true" aria-label="MCP" style={overlayStyle} onClick={onClose}>
    <div onClick={(e) => e.stopPropagation()} style={modalStyle}>
      ...
      <main style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
        <div style={mainHeaderStyle}><div style={{ flex: 1 }}><div style={{ fontSize: 15, fontWeight: 800, color: "var(--text)" }}>{selected?.name ?? "MCP 服务配置"}</div><div style={subStyle}>配置会保存到 ~/.pi/agent/mcp.json；后续可由 Agent 运行时读取接入。</div></div>{selected && <button onClick={remove} style={dangerBtnStyle}>删除</button>}<button onClick={save} disabled={saving} style={{ ...primaryBtnStyle, opacity: saving ? 0.5 : 1 }}>{saving ? "保存中..." : "保存"}</button><button onClick={onClose} style={closeBtnStyle}>×</button></div>
